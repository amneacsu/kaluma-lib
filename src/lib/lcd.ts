...
export class LCD {
  private rs: Pin;
  private en: Pin;
  private db: readonly [Pin, Pin, Pin, Pin];
  cols: number;
  rows: number;
  private charsz: 0 | 1;
  ...
  constructor(rs: Pin, en: Pin, db: readonly [Pin, Pin, Pin, Pin]) {
    this.rs = rs;
    this.en = en;
    this.db = db;
    this.cols = 16;
   ...
   */
  begin(cols?: number, rows?: number, charsz?: 0 | 1) {
  ...
  private write4bits(value: number) {
    for (var i = 0; i < 4; i++) {
