...
export class ST7735 {
  ...
  xstart: number;
  ystart: number;
  context: null | BufferedGraphicsContext | GraphicsContext;
  bufferedContext: null | BufferedGraphicsContext;

  constructor(spi: SPI, options: {
    width?: number;
    height?: number;
    dc: Pin;
    rst: Pin;
    ...
    this.xstart = options.xstart ?? 2;
    this.ystart = options.ystart ?? 3;
    this.bufferedContext = null;
    this.context = null;
  }
   ...
   */
  cmd(cmd: number, data?: Uint8Array | number[]) {
  ...
  getBufferedContext() {
    if (!this.bufferedContext) {
      this.bufferedContext = new BufferedGraphicsContext(this.width, this.height, {
  ...
  getContext() {
	   if (!this.context) {
      this.context = new GraphicsContext(this.width, this.height, {
        rotation: this.rotation,
        setPixel: (x, y, c) => {
          digitalWrite(this.cs, LOW); // select
